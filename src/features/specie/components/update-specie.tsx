...
import { NewSpecieFields } from "./specie-form-fields";
import { updateSpecieService } from "../server/db/specie-admin.service";
import { NewSpecieForm } from "./new-specie";
import { SpecieAdmin } from "../domain";
import {
    Tooltip,
    TooltipContent,
    TooltipTrigger,
...
export function UpdateSpecie({ specie }: { specie: SpecieAdmin }) {
    const queryClient = useQueryClient();
    const [open, setOpen] = useState(false);

    const form = useForm<NewSpecieForm>();
    ...
    const onSubmit = form.handleSubmit(async (data) => {
        try {
            await updateSpecieService(specie.id, {
        ...
        <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
            ...
            <DialogContent className="max-h-screen overflow-y-auto w-[95vw] sm:max-w-[60vw]">
                <DialogHeader>
                    ...
                    <DialogDescription>Modifica la información de la especie seleccionada.</DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    <form onSubmit={onSubmit} className="space-y-8 grid grid-cols-1 gap-2">
                        <NewSpecieFields showStatus />
