...
import { BuildingIcon, FileText, Activity, Settings } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { parseAsInteger, parseAsString, useQueryStates } from "nuqs";
import { getCompanyTypesPaginatedService } from "./server/db/company-type.service";
import { NewCompanyType } from "./components/new-company-type";
import { UpdateCompanyType } from "./components/update-company-type";
import { DeleteCompanyType } from "./components/delete-company-type";
import { TableCompanyType } from "./components/table-company-type";
...
export function CompanyTypeManagement() {
  ...
  const query = useQuery({
    queryKey: [COMPANY_TYPE_TAG, searchParams],
    queryFn: () =>
      getCompanyTypesPaginatedService({
    ...
    <div>
      <section className="mb-4 flex flex-col sm:flex-row sm:justify-between gap-2">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Administra los tipos de empresa para el registro de visitantes.
          </p>
        </div>
      ...
      <Card className="mb-4">
        <CardHeader>
          ...
          <CardDescription>
            Filtre los tipos de empresa por nombre o estado
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          },
          {
            ...
            cell: ({ row }) => (
              <div className="flex justify-center gap-x-2">
                <UpdateCompanyType companyType={row.original} />
                <DeleteCompanyType companyType={row.original} />
              </div>
            ),
          },
        ]}
