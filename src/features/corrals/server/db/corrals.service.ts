...
export function getLineByTypeService(lineaType: LineaType): Promise<ResponseLine> {
...
export function getCorralesByGroupService(groupId: number, date?: Date): Promise<ResponseCorrales> {
    ...
    if (date) {
        searchParams.date = date.toISOString().split('T')[0]; // Format as YYYY-MM-DD
    }
...
export async function getTotalAnimalsByLineService(idLine: number, admissionDate: Date | string): Promise<number> {
  const dateStr = typeof admissionDate === "string" ? admissionDate : admissionDate.toISOString().split("T")[0];
...
// Patch to close/open a corral by status record id
...
// Generate codes for all animals in the specified line
