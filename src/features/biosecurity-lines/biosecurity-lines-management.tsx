...
import { getBiosecurityLinesService } from "./server/db/biosecurity-lines.service";
...
export function BiosecurityLinesManagement() {
  ...
  const biosecurityLinesQuery = useQuery({
    ...
    queryFn: getBiosecurityLinesService,
  });

  const filteredBiosecurityLines = useMemo(() => {
    const items = biosecurityLinesQuery.data?.data ?? [];

    return items.filter((item) =>
      searchParams.idLine ? item.idLine === searchParams.idLine : false
    );
  }, [biosecurityLinesQuery.data, searchParams.idLine]);
  ...
  return (
    <div>
      ...
      {!searchParams.idLine ? (
      ...
      ) : filteredBiosecurityLines.length === 0 ? (
        <div className="py-16 text-center text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filteredBiosecurityLines.map((biosecurityLine) => (
            <BiosecurityLineCard
