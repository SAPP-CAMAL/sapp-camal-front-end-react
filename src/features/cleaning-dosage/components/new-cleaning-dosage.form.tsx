...
import { CleaningDosage } from "../domain/cleaning-dosage.types";
import {
  saveCleaningDosageService,
  updateCleaningDosageService,
...
import {
  ...
  getAllMethodsService,
} from "../server/db/cleaning-material.service";

interface NewCleaningDosageFormProps {
  trigger: React.ReactNode;
  isUpdate?: boolean;
  cleaningDosageData?: CleaningDosage & { id?: number };
  onSuccess?: () => void;
}

export default function NewCleaningDosageForm({
  trigger,
  isUpdate = false,
  onSuccess,
  cleaningDosageData,
}: NewCleaningDosageFormProps) {
  ...
  const initializeFormWithData = (data: any) => {
    const person: Person = {
      id: data.employeeId,
      fullName: data.employee.person.fullName,
    };

    setSelectedPerson(person);
    setFilterFullName(data.employee.person.fullName);
    setFilterIdentification("");
    setSelectedObservations(data.observation);
    setSelectedEquipment(data.equipment);
    setSelectedMaterial(data.idMaterial?.toString());
    setSelectedMethod(data.idMethod?.toString());
    setSelectedDose(data.dose);
  };
  ...
  useEffect(() => {
    if (open && isUpdate && cleaningDosageData) {
      initializeFormWithData(cleaningDosageData);
    }
  }, [open, isUpdate, cleaningDosageData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedEquipment) {
      toast.error("El campo equipo es obligatorio");
      return;
    }

    if (!selectedDose) {
      toast.error("El campo dosis es obligatorio");
      return;
    }
    ...
    try {
      if (isUpdate) {
        await updateCleaningDosageService(cleaningDosageData?.id ?? 1, {
          idEmployee: cleaningDosageData?.idEmployee ?? 0,
          dose: selectedDose,
          equipment: selectedEquipment,
          idMaterial: Number(selectedMaterial) ?? 1,
          idMethod: Number(selectedMethod) ?? 1,
          observation: selectedObservations,
        });
        toast.success("Registro actualizado correctamente");
      } else {
        await saveCleaningDosageService({
  ...
  const methodsList = useQuery({
    ...
    queryFn: getAllMethodsService,
    enabled: open,
  });

  const materialsList = useQuery({
    queryKey: ["cleaning-method"],
      ...
      <DialogContent className="min-w-[800px] max-h-[90vh] overflow-y-auto">
        ...
        >
          <CardHeader>
            ...
            <CardDescription>Llene el formulario.</CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-4 w-full">
              <div className="flex-1 flex flex-col">
                ...
                >
                  ...
                  <SelectContent>
                    {materialsList.data?.data.map((line, index) => (
              ...
              <div className="flex-1 flex flex-col">
                ...
                >
                  ...
                  <SelectContent>
                    {methodsList.data?.data.map((line, index) => (
                      <SelectItem key={index} value={String(line.id)}>
                        {capitalizeText(line.name)}
                      </SelectItem>
                    ))}
            ...
            <div className="flex flex-col w-full">
              ...
              <Input
                ...
                className="pl-3 pr-3 w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 h-10"
            ...
            <div className="flex flex-col w-full">
              <Label className="text-sm font-semibold">Dósis</Label>
              <Input
                type="text"
                placeholder="Ingrese Dósis..."
                className="pl-3 pr-3 w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 h-10"
                value={selectedDose}
