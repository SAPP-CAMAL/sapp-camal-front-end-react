...
export function TableCollectionValues<TData, TValue>({
  columns,
  data,
  isLoading,
  ...
  return (
    <div className="bg-white border rounded-lg overflow-hidden">
      <div className="py-4 px-4 flex flex-col border-b">
        <Label className="font-semibold text-lg lg:text-base">Tarifas por Especie</Label>
      </div>
                  ...
                  <CardContent className="p-4 space-y-2">
                    ...
                    {cellsById.specie && (
                      <div className="text-sm text-gray-500">
                        {flexRender(cellsById.specie.column.columnDef.cell, cellsById.specie.getContext())}
                      </div>
                    )}
                    {cellsById.code && (
                      <div className="text-sm text-gray-500">
                        {flexRender(cellsById.code.column.columnDef.cell, cellsById.code.getContext())}
                      </div>
                    )}
                    {cellsById.price && (
                      <div className="text-sm text-gray-500">
                        {flexRender(cellsById.price.column.columnDef.cell, cellsById.price.getContext())}
                      </div>
                    )}
                    {cellsById.actions && (
