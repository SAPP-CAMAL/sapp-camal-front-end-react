...
import {
  getCollectionValuesAdminService,
  getSpeciesAllService,
} from "./server/db/collection-value-admin.service";
...
export function CollectionValuesManagement() {
  ...
  const query = useQuery({
    queryKey: ["collection-values-admin", searchParams],
    queryFn: () =>
      getCollectionValuesAdminService({
      ...
      <Card className="mb-4 py-3 gap-2">
        <CardHeader className="px-4 gap-0.5">
          ...
          <CardDescription className="text-xs">
            Filtre las tarifas por nombre o especie
          </CardDescription>
        </CardHeader>
