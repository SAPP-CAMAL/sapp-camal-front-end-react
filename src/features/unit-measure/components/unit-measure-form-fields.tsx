...
export function NewUnitMeasureFields({ showStatus = false }: { showStatus?: boolean }) {
  const form = useFormContext<NewUnitMeasureForm>();
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex gap-2 items-center">
            <RulerIcon /> Unidad de Medida
          </CardTitle>
          <CardDescription>Define el código, nombre y símbolo de la unidad de medida.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-x-2 gap-y-4 items-start">
          ...
          <FormField
            control={form.control}
            name="symbol"
            rules={{ required: { value: true, message: "El símbolo es requerido" } }}
            render={({ field }) => (
              <FormItem>
                <FormLabel>Símbolo <RequiredMark /></FormLabel>
                <FormControl>
                  <Input maxLength={10} {...field} />
                </FormControl>
                <FormMessage />
