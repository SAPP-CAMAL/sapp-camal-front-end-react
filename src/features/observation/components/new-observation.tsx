...
import { createObservationService } from "../server/db/observation-admin.service";
...
const defaultValues: NewObservationForm = {
  name: "",
  status: "true",
};
...
export function NewObservation() {
  ...
  const form = useForm<NewObservationForm>({ defaultValues });
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      await createObservationService({
        name: data.name,
        status: true,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DialogContent className="max-h-screen overflow-y-auto w-[95vw] sm:max-w-[60vw]">
        <DialogHeader>
          <DialogTitle>Nueva Observación de Casillero</DialogTitle>
          <DialogDescription>Registra un nuevo tipo de observación para el control de casilleros.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={onSubmit} className="space-y-8 grid grid-cols-1 gap-2">
            <NewObservationFields />
