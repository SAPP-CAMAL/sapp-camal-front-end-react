...
import { createProvinceService } from "../server/db/locations-admin.service";
import { useEffect, useState } from "react";

export type NewProvinceForm = {
  code: string;
  name: string;
  status: string;
};

const defaultValues: NewProvinceForm = {
  code: "",
  name: "",
  status: "true",
};

export function NewProvince() {
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);

  const form = useForm<NewProvinceForm>({ defaultValues });
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      await createProvinceService({ code: data.code, name: data.name, status: true });
    ...
    <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DialogContent className="max-h-screen overflow-y-auto w-[95vw] sm:max-w-[60vw]">
        <DialogHeader>
          ...
          <DialogDescription>Define una nueva provincia.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
