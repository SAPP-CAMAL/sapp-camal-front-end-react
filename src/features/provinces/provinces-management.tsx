...
import { MapIcon, Activity, Settings } from "lucide-react";
...
export function ProvincesManagement() {
  ...
  const query = useQuery({
    queryKey: ["provinces-admin", { page, limit, name, status }],
    queryFn: () =>
      getProvincesAdminService({
        page,
        limit,
        ...(!!name && { name }),
        ...(status !== "*" && { status }),
    ...
    <div>
      <section className="mb-4 flex flex-col sm:flex-row sm:justify-between gap-2">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Jerarquía territorial (provincia, cantón, parroquia) usada en certificados
            y direcciones de destinatarios. Usa los botones de cada fila para navegar
            el siguiente nivel.
          </p>
        </div>
      ...
      <Card className="mb-4 py-3 gap-2">
        <CardHeader className="px-4 gap-0.5">
          ...
          <CardDescription className="text-xs">Filtre las provincias por nombre o estado</CardDescription>
        </CardHeader>
