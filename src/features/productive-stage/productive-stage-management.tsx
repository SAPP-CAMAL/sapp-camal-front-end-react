...
import { TableProductiveStage } from "./components/table-productive-stage";
...
export function ProductiveStageManagement() {
  ...
  const speciesById = useMemo(
    () => new Map((speciesResponse?.data ?? []).map((s) => [s.id, s.name])),
    [speciesResponse]
  );
  const animalSexById = useMemo(
    () => new Map((animalSexResponse?.data ?? []).map((a) => [a.id, a.name])),
    [animalSexResponse]
  );
    ...
    <div>
      <section className="mb-4 flex flex-col sm:flex-row sm:justify-between gap-2">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Administra las etapas productivas del animal por especie y sexo.
          </p>
        </div>
      ...
      <Card className="mb-4">
        <CardHeader>
          ...
          <CardDescription>
            Filtre las etapas productivas por nombre, código o estado
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <TableProductiveStage
        columns={[
          ...
          },
          {
            ...
            cell: ({ row }) => (
              <span>{speciesById.get(row.original.idSpecies) ?? "-"}</span>
            ),
          },
          {
            id: "animalSex",
            header: () => "Sexo",
            cell: ({ row }) => (
              <span>{animalSexById.get(row.original.idAnimalSex) ?? "-"}</span>
            ),
          },
          {
            accessorKey: "status",
          ...
          },
          {
            ...
            cell: ({ row }) => (
              <div className="flex justify-center gap-x-2">
                <UpdateProductiveStage productiveStage={row.original} />
                <DeleteProductiveStage productiveStage={row.original} />
              </div>
            ),
          },
        ]}
