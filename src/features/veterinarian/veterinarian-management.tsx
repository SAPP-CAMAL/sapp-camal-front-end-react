...
import { UserRoundIcon, Activity, Settings, ShieldCheckIcon } from "lucide-react";
...
export function VeterinarianManagement() {
  ...
  const query = useQuery({
    ...
    queryFn: getVeterinariansService,
  });
  ...
  const filteredData = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesName = searchParams.name
        ? item.fullName.toLowerCase().includes(searchParams.name.toLowerCase()) ||
    ...
    <div>
      <section className="mb-4 flex flex-col sm:flex-row sm:justify-between gap-2">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Administra el código, la autorización de distribución y el estado
            de los médicos veterinarios registrados en el sistema.
          </p>
        </div>
      </section>
      ...
      <Card className="mb-4">
        <CardHeader>
          ...
          <CardDescription>
            Filtre los médicos veterinarios por nombre, código o estado
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <TableVeterinarian
        columns={[
          ...
          {
            accessorKey: "authorizedDistribution",
            header: () => (
              <div className="flex items-center gap-2">
            ...
            cell: ({ row }) => (
              <Badge variant={row.original.authorizedDistribution ? "default" : "outline"}>
                {row.original.authorizedDistribution ? "Autorizado" : "No autorizado"}
              </Badge>
            ),
          },
          {
