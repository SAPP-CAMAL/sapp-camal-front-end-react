...
import { BugIcon, Hash, FileText, Activity, Settings } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { parseAsInteger, parseAsString, useQueryStates } from "nuqs";
import { getDiseasesPaginatedService } from "./server/db/disease.service";
import { NewDisease } from "./components/new-disease";
import { UpdateDisease } from "./components/update-disease";
import { DeleteDisease } from "./components/delete-disease";
import { TableDisease } from "./components/table-disease";
...
export function DiseaseManagement() {
  ...
  const query = useQuery({
    queryKey: [DISEASE_TAG, searchParams],
    queryFn: () =>
      getDiseasesPaginatedService({
    ...
    <div>
      <section className="mb-4 flex flex-col sm:flex-row sm:justify-between gap-2">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Administra el catálogo maestro de enfermedades detectables en
      ...
      <Card className="mb-4">
        <CardHeader>
          ...
          <CardDescription>
            Filtre las enfermedades por nombre, código o estado
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        columns={[
          {
            ...
            cell: ({ row }) => (
              <div className="flex items-center gap-x-2">
                {toCapitalize(row.original.names, true)}
              </div>
            ),
          },
          {
          ...
          },
          {
            ...
            cell: ({ row }) => (
              <div className="flex justify-center gap-x-2">
                <UpdateDisease disease={row.original} />
                <DeleteDisease disease={row.original} />
              </div>
            ),
          },
        ]}
