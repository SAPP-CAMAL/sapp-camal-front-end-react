...
export function TableCorrals<TData, TValue>({
  columns,
  data,
  meta,
  isLoading,
  ...
  return (
    <div className="bg-white border rounded-lg overflow-hidden">
      <div className="py-4 px-4 flex flex-col border-b">
        <Label className="font-semibold text-lg lg:text-base">Corrales</Label>
      </div>
              ...
              return (
                <Card key={row.id} className="overflow-hidden border-gray-200 py-0 gap-0">
                  <CardContent className="p-4 space-y-2">
                    ...
                    {cellsById.corralType && (
                      <div className="text-sm text-gray-500">
                        {flexRender(cellsById.corralType.column.columnDef.cell, cellsById.corralType.getContext())}
                      </div>
                    )}
                    {cellsById.quantity && (
                      <div className="text-sm text-gray-500">
                        {flexRender(cellsById.quantity.column.columnDef.cell, cellsById.quantity.getContext())}
                      </div>
                    )}
                    {cellsById.actions && (
