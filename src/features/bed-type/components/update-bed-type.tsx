...
import { updateBedType } from "../server/db/bed-type.service";
...
export function UpdateBedType({ bedType }: { bedType: BedType }) {
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);

  const form = useForm<NewBedTypeForm>();
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      ...
      toast.success("Tipo de cama actualizado exitosamente");
      setOpen(false);
    } catch (error: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DialogContent className="max-h-screen overflow-y-auto w-[95vw] sm:max-w-[50vw]">
        <DialogHeader>
          <DialogTitle>Editar Tipo de Cama</DialogTitle>
          <DialogDescription>
            Modifica la información del tipo de cama seleccionado.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
