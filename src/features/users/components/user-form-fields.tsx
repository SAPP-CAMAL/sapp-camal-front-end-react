...
import { PersonSearch, SelectedPersonCard } from '@/features/security/components/introductor-form-fields';
...
export function NewUserFields({ isUpdate = false }: { isUpdate?: boolean }) {
	...
	const [activeField, setActiveField] = useState<'name' | 'identification' | null>(null);
	...
	const isVeterinarianRole = form
		.watch('roles')
		.map(Number)
		.map((id: number) => query.data?.data.find(role => role.id === id)?.code)
		.includes(veterinarianCode);
						...
						<div className='grid grid-cols-2 gap-4 w-full'>
							<div className='space-y-2 relative'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel className='text-xs text-muted-foreground'>Buscar por nombre</FormLabel>
											<FormControl>
												<Input
													type='text'
							...
							<div className='space-y-2 relative'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel className='text-xs text-muted-foreground'>Buscar por identificación</FormLabel>
											<FormControl>
												<Input
													type='text'
								...
								{identification.length > 2 && activeField === 'identification' && (
									<div className='absolute z-50 w-full top-full mt-1'>
										<PersonSearch
											// @ts-ignore
											data={peopleData?.data?.data ?? []}
				...
				<>
					...
					<FormField
						...
						rules={{
							...
							validate: {
								matches: value => (value === form.watch('password') ? true : 'Las contraseñas no coinciden'),
							},
						}}
						render={({ field }) => (
							<FormItem>
							...
							<MultiSelect
								options={
									query.data?.data?.map(role => ({
										...
										label: toCapitalize(role.name, true),
									})) ?? []
								}
