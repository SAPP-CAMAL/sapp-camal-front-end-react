...
export function useSerialScale(config: SerialScaleConfig = {}) {
  ...
  const parseWeight = useCallback((data: string): WeightReading | null => {
    try {
      ...
      // Patrón específico de tu Bernalo X1
      // Formato detectado: =9.7400 donde los dígitos después del punto están invertidos
      // =9.7400 → 9 + "7400" invertido "0047" → 9.047? NO
      // =9.7400 → "97400" invertido "00479" → 479 / 10 = 47.9 ✓
      const bernaloMatch = cleaned.match(/^=(\d+)\.?(\d*)$/);
      
      if (bernaloMatch) {
        const wholePart = bernaloMatch[1]; // "9"
        const decimalPart = bernaloMatch[2] || ""; // "7400"
        
        console.log('🔍 Parsing:', cleaned, '→ wholePart:', wholePart, 'decimalPart:', decimalPart);
        
        // CASO 1: Formato =X.YYYY donde X es parte del número y YYYY son dígitos invertidos
        // Ejemplo: =9.7400 representa 47.9 kg
        if (decimalPart.length === 4) {
          // Concatenar wholePart + decimalPart e invertir todo
          const allDigits = wholePart + decimalPart; // "97400"
          const reversed = allDigits.split('').reverse().join(''); // "00479"
          console.log('🔄 Todos los dígitos invertidos:', allDigits, '→', reversed);
          
          // Convertir a número y dividir entre 10 para el decimal
          const numValue = parseFloat(reversed); // 479
          const valueInKg = numValue / 10; // 47.9
          
          console.log(`✅ Parsed Bernalo format (inverted all): ${cleaned} = ${valueInKg}kg`);
        ...
        // CASO 2: Formato sin punto o con más dígitos (ej: =975000 → 975 kg)
        const fullNumber = wholePart + decimalPart;
        const numericValue = parseInt(fullNumber);
        
        if (numericValue >= 1000) {
          // Son gramos, convertir a kg
          const valueInKg = numericValue / 1000;
          console.log(`✅ Parsed Bernalo format (grams): ${numericValue}g = ${valueInKg}kg`);
        ...
        // CASO 3: Valor directo en kg (para valores simples sin formato especial)
        const valueInKg = parseFloat(wholePart + '.' + (decimalPart || '0'));
        console.log(`✅ Parsed Bernalo format (kg direct): ${cleaned} = ${valueInKg}kg`);
      ...
      if (cleaned.startsWith('=') && cleaned.length < 5) {
        console.warn('⚠️ Lectura incompleta descartada:', cleaned);
        return null;
      }
      
      // Patrones alternativos (por si cambia el formato)
      // Patrón 1: Formato simple "  125.5 kg"
      let match = cleaned.match(/([+-]?\d+\.?\d*)\s*(kg|g|lb)/i);
      
      // Patrón 2: Formato con prefijo "ST,GS,+00125.5kg"
      if (!match) {
        match = cleaned.match(/ST,GS,([+-]?\d+\.?\d*)(kg|g|lb)/i);
      }
      
      // Patrón 3: Formato con WT: "WT:0125.5 KG"
      if (!match) {
        match = cleaned.match(/WT:([+-]?\d+\.?\d*)\s*(kg|g|lb)/i);
      }
      ...
      if (match) {
        ...
        // Detectar si el peso es estable (sin prefijo de movimiento)
        const stable = !cleaned.includes('US') && !cleaned.includes('unstable');
